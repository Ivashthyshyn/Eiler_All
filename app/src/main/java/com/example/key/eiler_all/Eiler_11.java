...
public class Eiler_11 {
    ...
    String number = "08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08" +
        ...
        int[] der = new int[number.length()]; //створюємо масив який міститиме числа типу long
        ...
        if (number.length() != 0) { // перетворюємо char в String
            ...
            int sum = 0;
            int z = 20; //змінна яка відображатиме суму перемноження рядка чисел
            for (int k = 3;k < z;k++ ){// цикл додаватиме по одному новому елементу рядка і перемножуватиме між собою

                int sumGorisontal = der[k-3] * der[k-2]* der[k-1]* der[k];
          ...
          int w = 401;
            for (int k = 30; k < w; k++ ){
                ...
                int sumVertical = der[w-30] * der[w-20]* der[w-10]* der[w];
                if (k == 19 & k == 39 & k == 59  & k == 79 & k == 99 ){
                    k = z;
                    z = z+20;
